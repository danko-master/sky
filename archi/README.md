...
Демонстрационный стенд: [НЕБО.ОБИТАТЬ.РФ] (http://небо.обитать.рф)
...
Обработчик может быть реализован на любом языке програмирования, но был выбран компилируемый язык Go (версия 1.25), как наиболее "легковесный" и быстрый. В одном потоке с памятью 24Gb обработан полный цикл (получен статус, прочитан файл из хранилища, распарсена каждая строка, найдены и сверены координаты в каждой строке, сохранены все данные в БД и отправлены все статусы) файла в 75 тысяч строк примерно за 3 минуты.
...
Реализован на компилируемом языке Go (версия 1.25).
...
\* топик для [целевой архитектуры](#2.), при масштабировании системы (в существующей версии не испоьзуется)
...
\* статусы для [целевой архитектуры](#2.), при масштабировании системы (в существующей версии не испоьзуются)
...
Реализован на Next.js (Node.js + React), который сочетает в себе преимущества разработки front-end на React и возможность серверной обработки данных в едином приложении.

> Администратор, загрузив файл для обработки имеет возможность отслеживать статус обработки в реальном времени. Отображается количество найденных записей. Добавление в БД происходит посредством дельт новых записей.
...
> Например, доступен рейтинг _Топ 10_ регионов, с максимальным количеством полётов.
...
| ------------------------------------------ | ----- | ---------------------------- | ---------------------------------------------------- |
| `/api/uploader`                            | POST  | `body`                       | Загрузка файла для обработки                         |
| `/api/helper`                              | GET   | -                            | Получение координат всех субъектов                   |
| `/api/report`                              | GET   | -                            | Общее количество полётов, с разбивкой по регионам    |
| `/api/report?bytime=monthly`               | GET   | -                            | Общее количество полётов, с разбивкой по месяцам     |
| `/api/report?bytime=weekly`                | GET   | -                            | Общее количество полётов, с разбивкой по неделям     |
| `/api/report?bytime=daily`                 | GET   | -                            | Общее количество полётов, с разбивкой по дням        |
| `/api/report?bytime=percentile&cont={num}` | GET   | `num` (значение 0 -100)      | Вычисление процентиля                                |
| `/api/downloader?uuid={uuid}`              | GET   | `uuid` (идентификатор файла) | Получение оригинального файла из файлового хранилища |
